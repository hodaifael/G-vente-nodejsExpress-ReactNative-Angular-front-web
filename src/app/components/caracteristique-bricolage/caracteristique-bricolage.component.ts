...
@Component({
  selector: 'app-caracteristique-bricolage',
  templateUrl: './caracteristique-bricolage.component.html',
  styleUrls: ['./caracteristique-bricolage.component.css']
})
export class CaracteristiqueBricolageComponent implements OnInit {
  ...
  insertProduct() {
    ...
    if (this.product.prixofficiel == 0) {
      ...
      this.insertnewproductreference();
    }
  }
  ...
  checkCaract(array: any) {
    for (var val of array) {
      if (val.vitesse == this.prod.vitesse && val.tension == this.prod.tension && val.alimentation == this.prod.alimentation) {
